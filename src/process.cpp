...
map<string, char*> *Process::GetLoadedModules() {
	...
	if (EnumProcessModules(this->pHandle, hMods, sizeof(hMods), &cbNeeded))	{
		for (i = 0; i < (cbNeeded / sizeof(HMODULE)); i++) {
			if (GetModuleFileNameExA(this->pHandle, hMods[i], szModName,	sizeof(szModName) / sizeof(TCHAR))) {
				filename = PathFindFileNameA(szModName);
...
string* Process::GetModuleFilename(void* base) {
	...
	if (GetModuleFileNameExA(this->pHandle, (HMODULE)base, szModName, sizeof(szModName) / sizeof(TCHAR))) {
